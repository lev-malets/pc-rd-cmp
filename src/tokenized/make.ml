...
module Make (Tokenizer : Sigs.TOKENIZER) (Conf : Pc.CONF): Sigs.TPC with type s = Conf.Log.elem and type tag = Tokenizer.Tag.t = struct
    module Id = struct
        let next = ref 0
    ...
    module Basic = struct
        ...
        let memo p =
            ...
            { p with
                ...
                p = { run = fun state fail succ ->
                    let table = Hashtbl.find_or_add state.im.memo_tables id ~default:(fun _ -> Hashtbl.create (module Int)) in
        ...
        let parse_string p ?filename text =
            let succ state v =
                if state.pos = Array.length state.im.tokens then Some v
                else None
            in
