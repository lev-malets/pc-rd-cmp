    ...
    let seq ?(n = 0) ?sep ?(trail = false) p =
      ...
      let info =
        ...
        | Consume p1, Consume p2 ->
            if p1.empty then
              Consume
                {empty = p1.empty && n = 1; first = Tset.union p1.first p2.first}
              ...
              (fun state fail succ ->
                ...
                | None ->
                    ...
                    let succ state v =
                      ...
                      succ state v
                    in
                    let fail state = set_data state None; fail state in
                    let state =
                      { state with
                        log = []
        ...
        ; p =
            { Parser.run =
                (fun state fail succ ->
                  ...
                  in
                  let succ state v = finalize state true; succ state v in
                  let fail state = finalize state false; fail state in
                  p.p.run state fail succ) } }
