...
module Make (Conf : Pc.CONF) : Sigs.POS with type s = Conf.Log.elem = struct
  ...
  module State = struct
    ...
    module Info = struct
      type t = {default_position : Lexing.position}
    end
  ...
  module Basic = struct
    ...
    let ( >> ) p1 p2 =
      ...
      | _ -> mk_parser_cont p1 p2 ~p:(p1.p >> p2.p) ~typ:Parser
    ...
    let make_position pos =
      ...
      let pos_lnum, pos_bol, pos_cnum = (line.no, line.start, pos) in
      let open Lexing in
      {info.default_position with pos_lnum; pos_bol; pos_cnum}
          ...
          { Angstrom.Expose.Parser.run =
              (fun input pos more fail succ ->
                ...
                let fail' input' _ more' _ _ =
                  State.set s; succ input' pos more' ()
                in
                p.p.run input pos more fail' succ') }
      ; info = Empty
      ; typ = Parser
    ...
    let seq ?(n = 0) ?sep ?(trail = false) p =
      ...
      { p =
          ...
          in
          list
          >>= fun list -> if List.length list < n then fail else return list)
      ; info
      ; typ = Parser
      ; id = Id.get () }
    ...
    let first_size_max = Charset.size Charset.full
    ...
    let first_iter ~f p =
      match p.info with
      | Consume {first; _} -> Charset.iter_code f first
      | _ -> failwith "check usage"
      ...
      ; p =
          { Angstrom.Expose.Parser.run =
              (fun input pos more fail succ ->
                ...
                in
                let succ input' pos' more' v =
                  finalize pos' true; succ input' pos' more' v
                in
                let fail input' pos' more' marks' msg' =
                  finalize pos' false;
                  fail input' pos' more' marks' msg'
    ...
    let eof =
      {p = Angstrom.end_of_input; id = Id.get (); info = Unknown; typ = Parser}
        ...
        (fun input pos more fail succ ->
          ...
          let rec loop lines start pos =
            ...
            | true -> (
              ...
              | '\t' -> loop lines start @@ (pos + 1)
              | '\r' -> (
                match pos + 1 < len && Char.(char (pos + 1) = '\n') with
                | true -> loop (lines + 1) (pos + 2) @@ (pos + 2)
                | false ->
                    let _ = failwith "here" in
                    fail input pos more [] "ws_pos")
  ...
  let whitespace =
    { p = whitespace
    ; info =
        Consume {empty = true; first = Charset.of_list [' '; '\n'; '\t'; '\r']}
  ...
  let with_literal p =
    { p with
      p =
        ...
        >>= fun _ ->
        let res = ref None in
        Angstrom.consumed (p.p >>| fun v -> res := Some v)
        >>| fun str ->
  ...
  let advance i =
    if i = 0 then
      {p = Angstrom.(advance 0); info = Empty; typ = Return (); id = Id.get ()}
    else
  ...
  let mk_first_test f =
    let rec loop set = function
      ...
      | i ->
          ...
          let newset = if f c then Charset.add set c else set in
          loop newset Int.(i + 1)
    in
