...
module FloatStatistics = struct
    ...
    let equal x y =
        Float.equal x.sum y.sum &&
        Float.equal x.min y.min &&
        Float.equal x.max y.max &&
        Int.equal x.count y.count
                ...
                let rec loop entries time =
                    ...
                    | x::xs as entries ->
                        if x.depth < depth then
                            ...
                            let stats =
                                Hashtbl.find_or_add table x.id
                                    ~default:begin fun _ ->
...
let last_pos =
    List.fold_left
        ~f:begin fun acc entry ->
            if gt acc entry.enter_pos then
                acc
            else
                entry.enter_pos
        end
