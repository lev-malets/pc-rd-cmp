...
struct
  ...
  type arrow_parser_helpers = { args_loop : expression Comb.t; types_loop : expression Comb.t }
  ...
  let x =
    ...
    (module struct
      ...
      let expression_constrainted =
        ...
        & choice [ with_loc & (hlp2 Exp.constraint_ + expression_fun - ng - colon - ng + core_type); expression_fun ]

      let value_binding_list =
        let value_binding =
          with_loc & ((hlp2 @@ Vb.mk ?docs:None ?text:None) + pattern - ng - eq - ng + expression_fun)
        in
      ...
      let letmodule =
        named "expression:module"
          (with_loc
          & (hlp3 Exp.letmodule - module' - ng + loc u_ident - ng - eq - ng + modexpr - del - ng + expression_sequence)
          )

      let pack =
        named "expression:pack"
        & choice
            [
              with_loc
              ...
              with_loc & (hlp Exp.pack - module' - ng - l_paren - ng + modexpr - ng - r_paren);
            ]
      ...
      let letopen =
        named "expression:open"
          (let override = choice [ bang >>$ Override; return Fresh ] in
           ...
           with_loc & (hlp3 Exp.open_ - open' + override - ng + loc u_longident - del - ng + expression_sequence))

      let letexception =
        named "expression:exception"
          (with_loc
          & (hlp2 Exp.letexception - exception' - ng + type_extension_constructor - del - ng + expression_sequence))

      let expression_in_braces =
        named "expression:in_braces" & choice [ choice [ let'; letmodule; letopen; letexception ]; expression_fun ]
      ...
      let expression_sequence =
        ...
        & choice
            [
              with_loc & (hlp2 Exp.sequence + expression_in_braces - del - ng + expression_sequence);
      ...
      let fun' =
        named "expression:arrow"
          (let arrow_tail =
             let tail = arrow >> ng >> expression_fun in
             choice
               [
                 mapping (fun typ expr -> Exp.constraint_ ~loc:expr.pexp_loc expr typ)
           ...
           let with_label =
             choice
               [
                 mapping (fun { txt; loc } pat -> (txt, pat_add_attr "ns.namedArgLoc" ~loc pat))
                 + label - ng - as' - ng + pattern_constrainted;
                 mapping (fun ({ txt; loc } as var) typ ->
                     ( txt,
                       Pat.constraint_ ~loc:(loc_comb loc typ.ptyp_loc) ~attrs:[ Hc.attr "ns.namedArgLoc" ~loc ]
                         (Pat.var ~loc var) typ ))
                 + label - ng - colon - ng + core_type_atom;
                 mapping (fun ({ txt; loc } as var) -> (txt, Pat.var ~loc ~attrs:[ Hc.attr "ns.namedArgLoc" ~loc ] var))
                 + label;
               ]
           in
           ...
           let with_value =
             choice
               [
                 mapping (fun (s, pat) -> (Optional s, pat, None)) + with_label - ng - eq - ng - question;
           ...
           let nolabel =
             choice
               [
                 with_loc & (hlp2 Pat.constraint_ + pat_attrs pattern - ng - colon - ng + core_type); pat_attrs pattern;
               ]
           ...
           let constr_unit =
             named "expression:arrow:unit"
               (let unit = l_paren >> r_paren >>$ Hc.lid [ "()" ] in
                ...
                mapping (fun p1 pat exp p2 -> Exp.fun_ ~loc:(loc_mk p1 p2) Nolabel None pat exp)
           ...
           let with_many_args =
             let loop =
               ...
               let curried =
                 choice
                   [
                     exp_attrs
                     & mapping (fun p1 (label, pat, expr) tail p2 -> Exp.fun_ ~loc:(loc_mk p1 p2) label expr pat tail)
               ...
               {
                 args_loop = choice [ dot >> ng >> curried >>| exp_add_attr "bs"; curried ];
                 types_loop =
                   choice
                     [
                       with_loc & (hlp2 Exp.newtype + loc l_ident - ng + types_loop);
                       comma >> ng >> exp_attrs (type' >> ng >> types_loop);
                       comma >> ng >> args_loop;
                     ];
               }
             ...
             named "expression:arrow:many_args"
             & choice
                 [
           ...
           let constr_unit_uncurried =
             let unit = return (Longident.Lident "()") - l_paren - ng - dot - ng - r_paren in
             let pat = loc unit >>| fun lid -> Pat.construct ~loc:lid.loc lid None in

             named "expression:arrow:unit_uncur"
               (mapping (fun p1 pat expr p2 ->
                    Exp.fun_ ~loc:(loc_mk p1 p2) ~attrs:[ Hc.attr "bs" ] Nolabel None pat expr)
               + pos + pat - ng + arrow_tail + pos)
           in
           ...
           let only_arg =
             named "expression:arrow:one_arg"
               (exp_attrs
               & mapping (fun p1 p e p2 -> Exp.fun_ ~loc:(loc_mk p1 p2) Nolabel None p e)
      ...
      let list_helper = make_list_helper ~constr:Exp.construct ~tuple:Exp.tuple ~get_loc:(fun x -> x.pexp_loc)
      ...
      let list =
        ...
        & choice
            [
              ...
              with_loc
              & mapping list_helper - list - ng + seq ~n:1 ~sep expression_constrainted
                + choice
                    [
      ...
      let case =
        ...
        + pattern - ng
        + opt (when' <|> if' >> ng >> expression)
        - ng - arrow - ng + expression_sequence

      let case_list = l_brace >> opt (ng >> pipe) >> seq ~n:1 (ng >> case) ~sep:(ng >> pipe) << ng << r_brace

      let switch = named "expression:switch" (with_loc & (hlp2 Exp.match_ - switch - ng + expression - ng + case_list))

      let try_catch =
        named "expression:try" (with_loc & (hlp2 Exp.try_ - try' - ng + expression - ng - catch - ng + case_list))
      ...
      let ifthenelse =
        ...
        named "expression:if"
          (with_loc & (hlp3 Exp.ifthenelse - if' - ng + expression - ng + scoped_sequence + opt else_part))

      let for_ =
        named "expression:for"
          (let direction = choice [ to' >>$ Upto; downto' >>$ Downto ] in
           let mapping = mapping (fun p1 pat ef dir et ea p2 -> Exp.for_ ~loc:(loc_mk p1 p2) pat ef et dir ea) in

           choice
             [
               mapping + pos - for' - ng + pattern - ng - in' - ng + expression - ng + direction - ng + expression - ng
               + scoped_sequence + pos;
               mapping + pos - for' - ng - l_paren - ng + pattern - ng - in' - ng + expression - ng + direction - ng
               + expression - ng - r_paren - ng + scoped_sequence + pos;
             ])

      let while_ =
        named "expression:while" (with_loc & (hlp2 Exp.while_ - while' - ng + expression - ng + scoped_sequence))

      let assert_ = named "expression:assert" (with_loc & (hlp Exp.assert_ - assert' - ng + expression))

      let lazy_ = named "expression:lazy" (with_loc & (hlp Exp.lazy_ - lazy' - ng + expression))
      ...
      let jsx =
        named "expression:jsx"
          (let arg =
             choice
               [
                 mapping (fun { txt; loc } expr -> (Labelled txt, exp_add_attr "ns.namedArgLoc" ~loc expr))
                 + loc l_ident - ng - eq - ng + expression_p0;
                 mapping (fun { txt; loc } expr -> (Optional txt, exp_add_attr "ns.namedArgLoc" ~loc expr))
           ...
           let children_list = with_loc & (mapping (fun list -> list_helper list None) + seq expression ~sep:ng) in
           ...
           choice
             [
               ...
               named "jsx:tag"
                 ...
                 & mapping (fun p1 tag args children tag2 p2 ->
                       ...
                       if String.(tag.txt <> tag2) then fail
                         ...
                         let tailargs = [ (Labelled "children", children); _unit_arg ] in
                         let e = Exp.apply ~loc:(loc_mk p1 p2) (Exp.ident tag) (args @ tailargs) in
                         return e)
                   + pos - lt - ng + loc l_ident
                   + seq (ng >> arg)
               ...
               named "jsx:ce:leaf"
                 ...
                 + pos - lt - ng + loc longident
                 + seq (ng >> arg)
                 - ng - slash - ng - gt + pos_end);
               named "jsx:ce:tag"
                 (run
                 & mapping (fun p1 tag args children tag2 p2 ->
                       let open Simple in
                       if Poly.(tag.txt <> tag2) then fail
                         ...
                         in
                         let e = Exp.apply ~loc:(loc_mk p1 p2) tag (args @ tailargs) in
                         return e)
                   + pos - lt - ng + loc longident
                   + seq (ng >> arg)
                   - ng - gt - ng + children - ng - lt - ng - slash - ng + longident - ng - gt + pos_end);
               named "jsx:notag:item"
                 (with_loc
                 & mapping (fun expr -> list_helper [ expr ] None)
                   - lt - ng - gt - ng - ellipsis - ng + expression - ng - lt - ng - slash - ng - gt);
               named "jsx:notag" (lt >> ng >> gt >> ng >> children_list << ng << lt << ng << slash << ng << gt);
             ])

      let jsx = jsx >>| exp_add_attr "JSX"

      let array =
        named "expression:array"
        & choice
            [
              mapping (fun p1 p2 -> Exp.array ~loc:(loc_mk p1 p2) []) + pos - l_bracket - ng - r_bracket + pos;
        ...
        named "expression:record"
          (let nb =
           ...
           & mapping (fun expr list loc -> Exp.record ~loc list expr)
             ...
             + seq ~n:1 nb ~sep ~trail:true - ng - r_brace)
        ...
        let record =
          let row =
            mapping (fun { txt; loc } pat ->
                ...
                match pat with Some pat -> (lid, pat) | None -> (lid, Exp.ident ~loc lid))
            + loc string_raw
            + opt (ng >> colon >> ng >> expression)
          in

          mapping (fun p1 list p2 -> Exp.record ~loc:(loc_mk p1 p2) list None)
          + pos - l_brace - ng + seq ~n:1 row ~sep ~trail:true - ng - r_brace + pos
        in
      ...
      let string = named "expression:string" (with_loc & (hlp Exp.constant + string_multiline))

      let js_string = named "expression:js_string" @@ template ~quote_tag:"js" ~expression

      let json_string = named "expression:json_string" & json_tag >> ng >> template ~quote_tag:"json" ~expression
      ...
      let polyvariant = named "polyvariant" (with_loc & (hlp2 Exp.variant + variant_tag + opt constr_args))

      let true_ = with_loc & (mapping (fun a loc -> Exp.construct ~loc a None) + loc (true' >>$ Longident.Lident "true"))

      let false_ =
        with_loc & (mapping (fun a loc -> Exp.construct ~loc a None) + loc (false' >>$ Longident.Lident "false"))

      let string_ident = with_loc & (hlp Exp.ident + loc (string_ident >>| fun x -> Longident.Lident x))
      ...
      let atom =
        choice ~name:"expression:atom"
          [
            ...
            false_;
            ident;
            with_loc & (hlp2 Exp.construct + loc u_longident + opt (ng_no_new_line >> constr_args));
            polyvariant;
            with_loc
            ...
            tuple;
            coerce;
            with_loc & parens & (mapping exp_loc + exp_attrs expression_constrainted);
            array;
      ...
      let object_set_cont =
        ...
        & mapping (fun idx p1 loc value prev ->
              let s = Exp.send ~loc:(loc_mk prev.pexp_loc.loc_start p1) prev idx in
          ...
          (let labelled special_arg =
             choice
               [
                 ...
                 mapping (fun tag expr -> (Labelled tag, expr)) - tilda - ng + l_ident - ng - eq - ng + special_arg;
                 mapping (fun { txt; loc } typ p2 ->
                     let exp =
                       Exp.constraint_ ~loc:(loc_mk loc.loc_start p2) ~attrs:[ Hc.attr ~loc "ns.namedArgLoc" ]
                 ...
                 mapping (fun { txt; loc } p2 ->
                     let exp =
                       Exp.ident ~loc:(loc_mk loc.loc_start p2) ~attrs:[ Hc.attr ~loc "ns.namedArgLoc" ]
                         (Location.mkloc (Hc.lid [ txt ]) loc)
                     in
                     (Optional txt, exp))
                 - tilda - ng + loc l_ident - ng - question + pos;
                 mapping (fun { txt; loc } ->
                     (Labelled txt, Hc.expr_id ~loc ~attrs:[ Hc.attr ~loc "ns.namedArgLoc" ] [ txt ]))
             ...
             named "expression:apply"
               (let flags = ref [] in
                ...
                let params =
                  choice
                    [
                      l_paren >> ng >> seq ~n:1 ~sep ~trail:true (labelled special_arg) << ng << r_paren;
           ...
           let apply_uncurried =
             named "expression:apply:uncurried"
                  ...
                  named "expression:apply:uncurried:tail"
                    (fold_left_cont_0_n
                       (mapping (fun args p2 expr ->
                            Exp.apply ~loc:{ expr.pexp_loc with loc_end = p2 } ~attrs:[ Hc.attr "bs" ] expr args)
                       + apply_params + pos)
           ...
           let field =
             ...
             >> named "expression:field"
                  (mapping (fun lid loc_end prev -> Exp.field ~loc:{ prev.pexp_loc with loc_end } prev lid)
           ...
           let object_get =
             ...
             >> named "expression:send"
                  (mapping (fun str loc_end prev -> Exp.send ~loc:{ prev.pexp_loc with loc_end } prev str)
           ...
           let array_get =
             ...
             >> named "expression:index"
                  (mapping (fun idx loc_end prev ->
                       Exp.apply ~loc:{ prev.pexp_loc with loc_end }
      ...
      let set =
        named "expression:set"
          (let set_cont = named "expression:set:cont" (field_set_cont <|> object_set_cont <|> array_set_cont) in
          ...
          [
            ...
            & mapping (fun op expr ->
                  ...
                  | Pexp_constant (Pconst_integer (str, suf) as c) ->
                      if Char.equal str.[0] '-' then fun loc -> Exp.constant ~loc c
                      else fun loc -> Exp.constant ~loc (Pconst_integer ("-" ^ str, suf))
                  | Pexp_constant (Pconst_float (str, suf) as c) ->
                      if Char.equal str.[0] '-' then fun loc -> Exp.constant ~loc c
                      else fun loc -> Exp.constant ~loc (Pconst_float ("-" ^ str, suf))
            ...
            & mapping (fun op expr ->
                  ...
                  | Pexp_constant (Pconst_float (str, suf)) -> fun loc -> Exp.constant ~loc (Pconst_float (str, suf))
                  | _ -> fun loc -> Exp.apply ~loc (Exp.ident ~loc:op.loc op) [ (Nolabel, expr) ])
              + loc (plus_dot >>$ "~+." <|> (plus >>$ "~+"))
              - ng + unary;
            with_loc
      ...
      let p0 = named "expression:p0" @@ exp_attrs (ifthenelse <|> unary)
      ...
      let left_assoc mp ops =
        let tail =
          mapping (fun op right left ->
              let op = str2lid op in
              Exp.apply ~loc:(loc_comb left.pexp_loc right.pexp_loc) (Exp.ident ~loc:op.loc op)
      ...
      let p2 = named "expression:p2" @@ left_assoc p1 (asterisk_asterisk >>$ "**")
      ...
      let p4 =
        ...
        @@ left_assoc2 p3
             (ng
             >> loc (plus_plus >>$ "^" <|> (plus_dot >>$ "+.") <|> (plus >>$ "+"))
             <|> (failed (ng_new_line >> (minus_dot <|> minus) >> failed ng_not_empty >> p0)
                 >> ng
                 >> loc (minus_dot >>$ "-." <|> (minus >>$ "-"))))
      ...
      let p6 = named "expression:p6" @@ left_assoc p5 (ampersand_ampersand >>$ "&&")

      let p7 = named "expression:p7" @@ left_assoc p6 (pipe_pipe >>$ "||")

      let p8 = named "expression:p8" @@ left_assoc p7 (hash_eq >>$ "#=" <|> (colon_eq >>$ ":="))

      let ternary =
        named "expression:ternary"
          (with_loc
          & mapping (fun e1 e2 e3 loc -> Exp.ifthenelse ~loc ~attrs:[ Hc.attr "ns.ternary" ] e1 e2 (Some e3))
            + p8 - ng - question - ng + expression_fun - ng - colon - ng + expression_fun)

      let expression = named "expression" (ternary <|> p8)
