...
module type TPC = Tokenized.Sigs.TPC
...
module Make (Tpc : TPC): Pc_syntax.Sigs.PARSE = struct
    module Base = struct
        ...
        let integer =
            named "tkn:integer" & tkn_payload Integer
        ...
        let string_raw =
            named "tkn:string" & tkn_payload String
        ...
        let single_line_comment =
            named "tkn:comment:s" & loc (tkn_payload Comment)
            >>| fun {txt; loc} -> Res_comment.makeSingleLineComment ~loc txt

        let multi_line_comment =
            named "tkn:comment:m" & loc (tkn_payload MultilineComment)
            >>| fun {txt; loc} -> Res_comment.makeMultiLineComment ~loc txt

        let comment =
            mapping begin fun pt_pos x ->
                Pc_syntax.Basic.LogElement.Comment (Res_comment.setPrevTokEndPos x pt_pos; x)
            end
