    ...
    (module struct
          ...
          fold_left_cont_0_1 expression
            (mapping (fun typ prev ->
                 Exp.constraint_
                   ...
                   prev typ)
            <<. colon <*>. core_type_package)
        in
        choice ~name:"modexpr:atom"
          [ with_loc
            & hlp Mod.unpack << unpack <<. l_paren <*>. unpack_constr
              <<. r_paren
      ...
      let apply =
        named "modexpr:apply"
          (let genarg =
             with_loc
             & mapping (fun loc -> Mod.structure ~loc []) << l_paren <<. r_paren
           in
        ...
        let arg_loop =
          ...
          let tail =
            choice ~name:"modexpr:functor:arg_loop:tail"
              [opt comma >> ng >> r_paren >> ng >> tail; comma >> ng >> arg_loop]
          in
      ...
      let structure =
        named "modexpr:structure"
          (with_loc & hlp Mod.structure << l_brace <*>. structure <<. r_brace)
