...
module Make (BasicBase : Sigs.BASIC_BASE) : Sigs.PARSER = struct
  ...
  let parsers =
    ...
    let payload =
      named "payload"
      & choice
          [ parens & mapping (fun x -> PPat (x, None)) << question <*>. pattern
          ; parens & mapping (fun x -> PSig x) << colon <<. sig' <*> signature
          ; parens & mapping (fun x -> PTyp x) << colon <*>. core_type
    ...
    let top_exception =
      ...
      & mapping (fun attrs constr ->
        ...
        <*> attrs_ << exception' <*>. type_extension_constructor
    in
    let top_include x =
      with_loc & hlp_a (Incl.mk ?docs:None) <*> attrs_ << include' <*>. x
    in
    let modtype_base =
      named "top:modtype" & with_loc
      & mapping (fun attrs name typ loc -> Mtd.mk ~attrs ~loc ~typ name)
        <*> attrs_ << module' <<. type' <*>. loc ident <<. eq <*>. modtype_with
    in
    let top_type mk =
      let first =
        mapping (fun attrs rec_flag decl ->
            ( rec_flag
      ...
      in
      let other =
        mapping (fun attrs decl ->
            {decl with ptype_attributes = attrs @ decl.ptype_attributes})
        <*> attrs_ << and' <*>. type_declaration
      in
      with_del
    ...
    let top_external =
      named "external"
        ...
        & mapping (fun attrs name typ prim loc ->
          ...
          <*> attrs_ << external' <*>. loc l_ident <<. colon << ng
          <*> core_type_poly <<. eq
         ...
         choice
           [ with_loc
             & mapping (fun attrs list name loc ->
               ...
               <*> attrs_ << import <*>. list <<. from << ng <*> loc string_raw
           ; with_loc
             & mapping (fun attrs item name loc ->
                   ...
                   Incl.mk ~loc ~attrs:(Hc.attr "ns.jsFfi" :: attrs) mod_)
               <*> attrs_ << import <*>. item <<. from << ng <*> loc string_raw
           ; with_loc
             & mapping (fun attrs list names loc ->
                   ...
                   let mod_ = Mod.structure str in
                   Incl.mk ~loc ~attrs:(Hc.attr "ns.jsFfi" :: attrs) mod_)
               <*> attrs_ << import <*>. list <<. from << ng <*> loc scope
           ; with_loc
             & mapping (fun attrs item names loc ->
                   ...
                   let mod_ = Mod.structure [Str.primitive @@ item attr] in
                   Incl.mk ~loc ~attrs:(Hc.attr "ns.jsFfi" :: attrs) mod_)
               <*> attrs_ << import <*>. item <<. from << ng <*> loc scope
           ; with_loc
    ...
    let value_str =
      named "str:value"
        (let lat = type' >> seq ~n:1 (ng >> loc l_ident) <<. dot in
         let with_lat =
           named "str:value:vb:lat"
             ...
             <*> pos <*> attrs_ <*> pattern <<. colon <*>. lat << ng
             <*> core_type_atom <<. eq <*>. expression_fun)
         in
         let helper =
           choice
             [ with_lat
             ; mapping (fun pattern expr attrs loc ->
               ...
               <*> pattern_poly_constrainted <<. eq << ng <*> expression_fun ]
         in
         let first =
           choice
             [ with_loc
               & mapping (fun attrs rec_flag decl loc ->
         ...
         in
         let other =
           mapping (fun attrs p1 x p2 ->
               let decl = x attrs in
               decl @@ loc_mk p1 p2)
           <*> attrs_ << and' <*>. pos <*> helper <*> pos
         in
         with_del
