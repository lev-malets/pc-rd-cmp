...
module Make (BasicBase : Sigs.BASIC_BASE) : Sigs.PARSER = struct
  ...
  let parsers =
    ...
    let module Core : CORE with module Comb = Comb = struct
      ...
      let pat_attrs p =
        mapping (fun loc_start attrs x ->
            match attrs with
            | [] -> x
            | _ -> { x with ppat_attributes = x.ppat_attributes @ attrs; ppat_loc = { x.ppat_loc with loc_start } })
        + pos + attrs_ + p
      ...
      let exp_attrs p =
        mapping (fun loc_start attrs x ->
            match attrs with
            | [] -> x
            | _ -> { x with pexp_attributes = x.pexp_attributes @ attrs; pexp_loc = { x.pexp_loc with loc_start } })
        + pos + attrs_ + p
      ...
      let mod_attrs p =
        mapping (fun loc_start attrs x ->
            match attrs with
            | [] -> x
            | _ -> { x with pmod_attributes = x.pmod_attributes @ attrs; pmod_loc = { x.pmod_loc with loc_start } })
        + pos + attrs_ + p
      ...
      let mty_attrs p =
        mapping (fun loc_start attrs x ->
            match attrs with
            | [] -> x
            | _ -> { x with pmty_attributes = x.pmty_attributes @ attrs; pmty_loc = { x.pmty_loc with loc_start } })
        + pos + attrs_ + p
      ...
      let typ_attrs p =
        mapping (fun loc_start attrs x ->
            match attrs with
            | [] -> x
            | _ -> { x with ptyp_attributes = x.ptyp_attributes @ attrs; ptyp_loc = { x.ptyp_loc with loc_start } })
        + pos + attrs_ + p
    end in
    ...
    let payload =
      ...
      & choice
          [
            parens & (mapping (fun x -> PPat (x, None)) - question - ng + pattern);
    ...
    let module Expression = Parser_expression.Make (Basic) (Core) (Type) (Pattern) (Modexpr) in
    let module Modtype = Parser_modtype.Make (Basic) (Core) (Type) (Expression) (Modexpr) in
    let module Modexpr = Parser_modexpr.Make (Basic) (Core) (Type) (Expression) (Modtype) in
    let open Modtype in
    let open Modexpr in
    let open Expression in
    let module_extension = named "module extension" @@ id_payload_pair percent_percent in

    let module_attribute = named "module attribute" @@ id_payload_pair at_at in

    let top_open =
      named "open" & with_loc
      & mapping (fun attrs override name loc -> Opn.mk ~loc ~attrs ?docs:None ?override name)
        + attrs_ - open'
        + opt (bang >>$ Override)
        - ng + loc u_longident
    in
    ...
    let top_include x = with_loc & (hlp_a (Incl.mk ?docs:None) + attrs_ - include' - ng + x) in
    ...
    let top_type mk =
      let first =
        mapping (fun attrs eflag rec_flag decl ->
            let decl = match eflag with None -> decl | Some loc -> tdecl_add_attr "genType" ~loc decl in
      ...
      with_del
      & (mapping (fun (rec_flag, hd) tail loc -> mk ?loc:(Some loc) rec_flag (hd :: tail)) + first + seq (ng >> other))
    in

    let top_external =
      named "external"
        (with_loc
        & mapping (fun attrs name typ prim loc -> Val.mk ~loc ~attrs ?docs:None ~prim name typ)
          + attrs_ - external' - ng + loc l_ident - ng - colon - ng + core_type_poly - ng - eq
    ...
    let top_module = named "module" & with_loc & attrs_ && module' >> ng >> module_decl in

    let top_module_rec =
      named "module rec"
        (let first = with_loc & attrs_ && module' >> ng >> rec' >> ng >> module_decl in
         let other = with_loc & attrs_ && and' >> ng >> module_decl in

         with_loc & (mapping (fun hd tail loc -> Sig.rec_module ~loc (hd :: tail)) + first + seq (ng >> other)))
    in
    ...
    let top_import =
      named "import"
        (let item =
         ...
         let scope =
           seq ~n:1 ~sep:(ng - dot - ng) (with_loc & (mapping (fun x loc -> Exp.constant ~loc (Const.string x)) + ident))
         in
             ...
             & mapping (fun attrs list name loc ->
                   let attr =
                     ( Location.mkloc "genType.import" name.loc,
                       PStr [ Str.eval @@ Exp.constant @@ Const.string name.txt ] )
                   in
                     ...
                     ( Location.mkloc "genType.import" name.loc,
                       PStr
                         [
                           Str.eval
                           @@ Exp.tuple
                                [ Exp.constant @@ Const.string name.txt; Exp.constant @@ Const.string "default" ];
                         ] )
                   in
        ...
        (seq ~sep:ng
           (choice ~name:"sig:elem"
              [
                ...
                named "sig:exception" & with_del & (na_hlp Sig.exception_ + top_exception);
                with_del & (na_hlp Sig.value + top_external);
                named "sig:include" & with_del & (na_hlp Sig.include_ + top_include modtype_with);
        ...
        (let lat = type' >> seq ~n:1 (ng >> loc l_ident) << ng << dot in
         ...
         let with_lat =
           named "str:value:vb:lat"
             (mapping (fun p1 attrs pat types typ expr vb_attrs vb_loc ->
                              ...
                              let desc =
                                ...
                                | [] ->
                                    let rec loop = function
                                      ...
                                      | x :: xs -> (
                                          match t.txt with
                                          | Longident.Lident t when String.equal t x.txt -> Ptyp_var x.txt
                                          | _ -> loop xs)
                                    in
                                    loop types
                                | _ -> Ptyp_constr (t, a)
                  ...
                  let expr = List.fold_right types ~f:Exp.newtype ~init:(Exp.constraint_ expr typ) in
                  Vb.mk ~loc:vb_loc ~attrs:vb_attrs pat expr)
             + pos + attrs_ + pattern - ng - colon - ng + lat - ng + core_type_atom - ng - eq - ng + expression_fun)
         in
         ...
         let helper =
           choice
             [
               with_lat;
               mapping (fun pattern expr attrs loc -> Vb.mk ~loc ~attrs pattern expr)
           ...
           choice
             [
               ...
               & mapping (fun attrs eflag rec_flag decl loc ->
                     let decl =
                       match eflag with None -> decl attrs | Some loc -> decl (Hc.attr "genType" ~loc :: attrs)
                     in
                     match rec_flag with None -> (Nonrecursive, decl loc) | _ -> (Recursive, decl loc))
                 + attrs_
                 + opt (loc_of export - ng)
                 - let' - ng
                 + opt (rec' << ng)
         ...
         let other =
           mapping (fun attrs p1 eflag x p2 ->
               let decl = match eflag with None -> x attrs | Some loc -> x (Hc.attr "genType" ~loc :: attrs) in
               decl @@ loc_mk p1 p2)
           + attrs_ - and' - ng + pos
           + opt (loc_of export - ng)
           + helper + pos
         ...
         with_del
         & mapping (fun (rec_flag, first) others loc -> Str.value ~loc rec_flag (first :: others))
           + first
           + seq (ng >> other))
    in
    ...
    let module_binding = named "str:mb" (with_loc & attrs_ && module' >> ng >> module_binding_helper) in

    let rec_module_binding =
      let first = with_loc & attrs_ && module' >> ng >> rec' >> ng >> module_binding_helper in
      ...
      & seq ~sep:ng
          (choice ~name:"str:elem"
             [
               ...
               named "str:exception" & with_del & (na_hlp Str.exception_ + top_exception);
               with_del & (na_hlp Str.primitive + top_external);
               with_del & (na_hlp Str.include_ + top_import);
               named "str:include" & with_del & (na_hlp Str.include_ + top_include modexpr);
               with_del & (na_hlp Str.module_ + module_binding);
               named "str:modtype" & with_del & (na_hlp Str.modtype + modtype_base);
  ...
  let parse p ~src ~filename =
    let fold_f (c, d) = function LogElement.Comment x -> (x :: c, d) | Diagnostics x -> (c, x :: d) in

    let p =
      mapping (fun x (comments, diagnostics) ->
          Res_driver.{ filename; source = src; parsetree = x; diagnostics; invalid = false; comments })
      + p
      + fold_log ([], []) fold_f
    in
