      ...
      let modtype_atom =
        choice ~name:"modtype:atom"
          [
            ...
            with_loc
            & (hlp Mty.signature - l_brace - ng + signature - ng - r_brace);
            with_loc & (hlp Mty.extension + extension);
            with_loc
            & (hlp Mty.typeof_ - module' - ng - type' - ng - of' - ng + modexpr);
        ...
        let with_constraint =
          ...
          & choice
              [
                ...
                - module' - ng + loc u_longident - ng - eq - ng
                + loc u_longident;
                mapping (fun m1 m2 -> Pwith_modsubst (m1, m2))
                - module' - ng + loc u_longident - ng - colon_eq - ng
                + loc u_longident;
              ]
        in

        with_loc
        & hlp2 Mty.with_ + modtype_functor - ng - with'
