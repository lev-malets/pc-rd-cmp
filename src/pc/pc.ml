...
module Utils = struct
  let mk_regexp xs =
    let rec loop = function [] -> "" | [ x ] -> "\\(" ^ x ^ "\\)" | x :: xs -> "\\(" ^ x ^ "\\)\\|" ^ loop xs in
    Str.regexp @@ loop xs
  ...
  module MakeConf
    ...
    let memoize =
      match member "memoize" json with `Null -> empty_regexp | x -> to_list x |> List.map ~f:to_string |> mk_regexp

    let trace =
      match member "trace" json with `Null -> empty_regexp | x -> to_list x |> List.map ~f:to_string |> mk_regexp

    let peek =
      match member "peek" json with `Null -> empty_regexp | x -> to_list x |> List.map ~f:to_string |> mk_regexp
  end

  let check_string regexp str = Str.string_match regexp str 0 && Str.match_end () = String.length str
end

module Make (Basic : COMB_BASE) (Conf : CONF with type Log.elem = Basic.log_elem) :
  COMB with type 'a t = 'a Basic.t and type 'a Simple.t = 'a Basic.Simple.t and type log_elem = Basic.log_elem = struct
  include Basic
  ...
  let fold_left_0_n ~f nil p =
    ...
    modify hlp
      ~simple:
        (let open Simple in
        let rec loop acc = simple p >>= (fun x -> loop @@ f acc x) <|> return acc in
        simple nil >>= loop)
  ...
  let fold_left_cont_0_n nil cont =
    let tail =
      fix @@ fun tail ->
      mapping (fun cont tail prev -> match tail with None -> cont prev | Some tail -> tail (cont prev))
      + cont + opt tail
    in

    mapping (fun x tail -> match tail with None -> x | Some tail -> tail x) + nil + opt tail

  let fold_left_cont_0_1 nil cont =
    mapping (fun x cont -> match cont with None -> x | Some cont -> cont x) + nil + opt cont

  let ( && ) v f = mapping (fun v f -> f v) <*> v <*> f

  let loc p = mapping (fun p1 x p2 -> Location.mkloc x @@ loc_mk p1 p2) + pos + p + pos_end
  ...
  let rec name_of_id id =
    ...
    | Some id ->
        ...
        name ^ "*"
    | None -> ( match Hashtbl.find id2name id with Some x -> x | None -> "_unnamed_")
  ...
  let choice ?name ps =
    match name with
    | None ->
        let rec loop = function [] -> failwith "check usage" | [ x ] -> x | x :: xs -> x <|> loop xs in
        loop ps
    | Some name ->
        let p =
